...
class AutoDoc(object):
        ...
        if self.response_schemas:
            ...
            for status_code in self.response_schemas:
                ...
                if isinstance(schema, str):  # is only text description
                    status_code_dict = {status_code: {"description": schema, "content": {}}}
                else:
                    schema_name = get_schema_name(schema)
                    if not inspect.isclass(schema) and schema.many:
                        schema_body = {
                            "type": "array",
                            "items": {"$ref": "#/components/schemas/{}".format(schema_name)},
                        }
                    else:
                        schema_body = {"$ref": "#/components/schemas/{}".format(schema_name)}
    ...
    def _generate_request_body(self):
        ...
        if body_schema:
            request_body_dict = {
                "content": {
                    self.content_type: {
                        "schema": {"$ref": "#/components/schemas/{}".format(get_schema_name(body_schema))}
                    }
                }
            }
    ...
    def _add_security(self):
        if self.security:
            self.view_operations[self.method.lower()].update({"security": [{self.security: []}]})
